...
class LootTableEditorView extends StatefulWidget {
  const LootTableEditorView({super.key});

  @override
  State<LootTableEditorView> createState() => _LootTableEditorViewState();
}

class _LootTableEditorViewState extends State<LootTableEditorView> with AutomaticKeepAliveClientMixin<LootTableEditorView> {
  LootTableModel? _lootTable;
  ...
  bool _parseJSONToLootTable(String jsonStr) {
    try {
      var jsonDec = jsonDecode(jsonStr);
      if(jsonDec != null) {
        _lootTable = LootTableModel.fromJson(jsonDec);
  ...
  void _onLootTableDataUpdate() {
    _parseLootTableToJSON();
    _autosave(_jsonTextFieldController.text);
  }

  bool _parseLootTableToJSON() {
    _jsonTextFieldController.text = jsonEncode(_lootTable!.toJson());
  ...
  Future<bool> _recoverLootTable() async {
    bool hasLootTable = false;
    if(_lootTable != null) {
      return true;
    }
                            ...
                            children: [
                              JsonTextField(
                                ...
                                decoration: InputDecoration(
                                  ...
                                  hintText: "To load a loot table, paste the JSON here."
                                ),
                                onChanged: (value) {
                                  if(_parseJSONToLootTable(value)) {
                                    _autosave(value);
                                  }
                                },
                                  ...
                                  children: [
                                    ...
                                    IconButton(
                                      ...
                                      onPressed: () {
                                        Clipboard.setData(ClipboardData(text: _jsonTextFieldController.text));
                                        try {
                                          exportStringAsJson(_jsonTextFieldController.text, _lootTable!.lootTable);
                                        }
                                        catch(e) {
                                          toastification.show(
                                            context: context,
